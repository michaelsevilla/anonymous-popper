...
% how i used it
\noindent\textbf{CudeleFS}: uses the metadata store format to write objects
...
the updates from a client's decoupled namespace ({\it i.e.} merge).  These
implementations required no changes to CephFS because the metadata servers know
how to read the events the library is writing into the object store.  By
...
\noindent\textbf{CudeleFS}: uses the journal subsystems within the metadata
...
\noindent\textbf{Used to Implement}: RPCs, Stream, Volatile Apply, Local/Persist
...
\noindent\textbf{CudeleFS}: using the journal tool, clients can materialize the
...
\noindent\textbf{Used to Implement}: Create, Volatile/Nonvolatile Apply
...
\noindent\textbf{Used to Implement}: Nonvolatile Apply, Create
