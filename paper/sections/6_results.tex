...
mechanism and bars below are faster.  ``Stream" is an approximation of the
...
workoad we tested is 100K file creates in the same directory.  This is the
maximum recommended size of a directory in CephFS; preliminary experiemnts with
...
be scanned for every operation.
...
Figure~\ref{fig:slowdown-strong-weak} shows the runtimes of systems employing
...
Consistency, No Fault Tolerance = ``RPCs"; Weak Consistency = ``Append Client Journal"s" +
``Local Persist"; and Weak Consistency, No Fault Tolerance = ``Append Client Journal"s" +
